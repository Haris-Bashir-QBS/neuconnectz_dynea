...
class OutboundDeliverySalesItemsPage extends StatelessWidget {
  final OutboundDeliverySalesItemsPageParams params;

  const OutboundDeliverySalesItemsPage({super.key, required this.params});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<OutboundDeliverySalesBloc>(),
      child: _OutboundDeliverySalesItemsView(params: params),
    );
  }
}

class _OutboundDeliverySalesItemsView extends StatefulWidget {
  final OutboundDeliverySalesItemsPageParams params;

  const _OutboundDeliverySalesItemsView({required this.params});

  @override
  State<_OutboundDeliverySalesItemsView> createState() =>
      _OutboundDeliverySalesItemsViewState();
}

class _OutboundDeliverySalesItemsViewState
    extends State<_OutboundDeliverySalesItemsView> {
  ...
  void _loadPending({required bool refresh}) {
    context.read<OutboundDeliverySalesBloc>().add(
      LoadOutboundDeliverySalesItemsEvent(
        params: OutboundDeliverySalesItemParams(
  ...
  void _loadCompleted({required bool refresh}) {
    context.read<OutboundDeliverySalesBloc>().add(
      LoadCompletedOutboundDeliverySalesItemsEvent(
        params: OutboundDeliverySalesItemParams(
          deliveryNo: widget.params.delivery,
          plant: widget.params.plant,
          storageLocation: widget.params.storageLocation,
  ...
  void _onPendingScroll() {
    if (_pendingScrollController.position.pixels ==
      ...
      final bloc = context.read<OutboundDeliverySalesBloc>();
  ...
  Future<void> _showCompletedDetails(
    ...
    await context.pushNamed(
      AppRoutes.completedOutboundDeliverySalesItemDetail,
      extra: item,
    );
    if (!mounted) return;
    _loadCompleted(refresh: true);
