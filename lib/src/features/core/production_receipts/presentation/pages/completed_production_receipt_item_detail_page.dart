...
class CompletedProductionReceiptItemDetailPage extends StatelessWidget {
  final ProductionReceiptItemEntity item;

  const CompletedProductionReceiptItemDetailPage({
    super.key,
    required this.item,
  });
  ...
  double get _remainingQuantity =>
      max(0, item.trQuantity - _totalBinQuantity.toDouble());
        ...
        children: [
          Expanded(
            child: CustomTextFormField(
              label: "Quantity",
              readOnly: true,
              initialValue: item.trQuantity.formatWithCommas,
              fillColor: AppPalette.lightGreyColor,
              enabled: false,
            ),
          ),
  ...
  Widget _binDetailTile(ProductionReceiptItemBinDetailEntity bin) {
    return Padding(
