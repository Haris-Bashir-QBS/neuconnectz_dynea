...
import 'package:neuconnectz_dynea/src/features/core/purchase_receipts/presentation/widgets/grn_row_widget.dart';
...
class PurchaseOrderGrnItemsPage extends StatelessWidget {
  final PurchaseOrderGrnItemsPageParams params;

  const PurchaseOrderGrnItemsPage({super.key, required this.params});
...
class _GrnItemsViewState extends State<_GrnItemsView> {
  ...
  void _loadPendingData() {
    ...
    context.read<PurchaseOrderGrnBloc>().add(
      LoadPurchaseOrderGrnItemsEvent(params: params, refresh: true),
    );
  }
  ...
  void _onPendingScroll() {
    if (_pendingScrollController.position.pixels ==
      ...
      if (state.pendingSection.hasMore && !state.pendingSection.isLoadingMore) {
        ...
        context.read<PurchaseOrderGrnBloc>().add(
          LoadPurchaseOrderGrnItemsEvent(params: params, refresh: false),
        );
      }
    }
  }
  ...
  void _loadCompletedData({bool refresh = true}) {
    ...
    context.read<PurchaseOrderGrnBloc>().add(
      LoadCompletedPurchaseOrderGrnItemsEvent(params: params, refresh: refresh),
    );
  }
  ...
  Future<void> _showQuantityBottomSheet(PurchaseOrderGrnItemEntity item) async {
    final result = await context.pushNamed<bool>(
      AppRoutes.grnQuantity,
      extra: PurchaseOrderGrnQuantityPageParams(
  ...
  Widget _buildPendingList(PurchaseOrderGrnItemsSectionState pendingState) {
  ...
  Widget _buildCompleteList(PurchaseOrderGrnItemsSectionState completedState) {
