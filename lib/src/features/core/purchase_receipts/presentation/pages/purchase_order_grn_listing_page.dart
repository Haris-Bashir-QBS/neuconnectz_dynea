...
import 'package:neuconnectz_dynea/src/features/core/purchase_receipts/presentation/widgets/grn_list_shimmer.dart';
import 'package:neuconnectz_dynea/src/features/core/purchase_receipts/presentation/widgets/grn_widget.dart';
...
class PurchaseOrderGrnListingPage extends StatelessWidget {
  final PurchaseOrderGrnListingPageParams params;

  const PurchaseOrderGrnListingPage({super.key, required this.params});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<PurchaseOrderGrnBloc>(),
...
class __GrnListingViewState extends State<_GrnListingView> {
  ...
  void _loadInitialData() {
    ...
    context.read<PurchaseOrderGrnBloc>().add(
      LoadPendingPurchaseOrderGrnEvent(params: params, refresh: true),
    );
  }
  ...
  void _onScroll() {
    ...
    if (pixels >= maxScroll - 200) {
      final state = context.read<PurchaseOrderGrnBloc>().state;

      if (state is PurchaseOrderGrnHeaderListFetched &&
          state.hasMore &&
          !state.isLoadingMore) {
        final params = PurchaseOrderGrnListParams(
          plant: widget.params.plant.code,
        ...
        context.read<PurchaseOrderGrnBloc>().add(
          LoadPendingPurchaseOrderGrnEvent(params: params, refresh: false),
        );
      }
    }
  }
  ...
  Widget _buildPendingList() {
    return BlocConsumer<PurchaseOrderGrnBloc, PurchaseOrderGrnState>(
      listener: (context, state) {
        if (state is PurchaseOrderGrnHeaderListFetchFailure) {
      ...
      builder: (context, state) {
        if (state is PurchaseOrderGrnHeaderListLoading) {
          return Column(
            children: [
  ...
  void _navigateToGrnItemsListingPage(
    ...
    PurchaseOrderGrnHeaderListFetched state,
    int index,
  ) async {
    await context
        .pushNamed(
          AppRoutes.grnItems,
          extra: PurchaseOrderGrnItemsPageParams(
