...
class PurchaseOrderGrnQuantityPage extends StatelessWidget {
  final PurchaseOrderGrnQuantityPageParams params;

  const PurchaseOrderGrnQuantityPage({super.key, required this.params});

  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => sl<PurchaseOrderGrnPutAwayBloc>()),
        BlocProvider(create: (_) => sl<BinBloc>()),
      ],
      child: _GrnQuantityView(params: params),
    );
  }
}

class _GrnQuantityView extends StatefulWidget {
  final PurchaseOrderGrnQuantityPageParams params;

  const _GrnQuantityView({required this.params});

  @override
  State<_GrnQuantityView> createState() => _GrnQuantityViewState();
}

class _GrnQuantityViewState extends State<_GrnQuantityView> {
  ...
  void _loadBins({
    ...
    context.read<BinBloc>().add(
      LoadBinsEvent(
        warehouseCode: widget.params.grn.warehouseNumber,
        storageType: storageType ?? widget.params.grn.destStorageType,
        keyword: keyword,
        lastCount: _pageSize,
  ...
  Widget build(BuildContext context) {
    ...
    >(
      ...
      builder: (context, state) {
        final isSubmitting = state is CreatePurchaseOrderGrnPutAwayLoading;
        return Scaffold(
                      ...
                      _dialogHasMore
                          ? () {
                            _loadBins(
                              ...
                              storageType: widget.params.grn.destStorageType,
                            );
                          }
                          : null,
                );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          CustomTextFormField(
            label: "Material Name",
            initialValue: widget.params.item.materialDescription,
            readOnly: true,
            isMarquee: true,
